...
if (isset($_COOKIE['email']) && !isset($_SESSION['google_signin_processed'])) {
   $email = $_COOKIE['email'];
   unset($_COOKIE['email']); 
   setcookie('email', '', -1, '/'); 
   ...
   $insert = $conn->prepare("INSERT INTO users(email, user_type) VALUES(?, ?)");
   $insert->execute([$email, "user"]);
   
   $_SESSION['user_id'] = $rowCount+1;
   header('location:home.php');
   exit;
   }
}
...
<body>
   ...
   <div style="display: flex; justify-content: center; align-items: center; height: 300px; ">
      <img src="images/kisanlogo.JPG" style="max-width: 100%; max-height: 100%;  " alt="Kisan Logo">
   </div>
